...
namespace FileSystem.EF.DbModels
{
    public partial class FolderDbModel : BaseEntity<int>, IFolder
    {
        ...
        public void DeleteSubFolderById(int subfolderId) 
            =>GetSubFolderById(subfolderId).Delete();
        

        public IFile CreateFile(string fileName)
        {
            var newFile = new FileDbModel(fileName, this);
            Files.Add(newFile);
            return newFile;
        }
        ...
        {
            var fileToDelete = Files.FirstOrDefault(f => f.Equals(file));
            fileToDelete?.Delete();
        }
        ...
        private FolderDbModel GetSubFolderById(int id)
            => Subfolders.Single(sf => sf.Id == id);
        ...
        private HierarchyId GetNewChildNode()
            => Node.GetDescendant(GetMaxChildNode(), null);

        private HierarchyId? GetMaxChildNode()
            => Subfolders.Max(sf => sf.Node);
